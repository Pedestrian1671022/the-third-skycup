...
def track_iou(detections, tracks_active, sigma_l, sigma_h, sigma_iou, t_life, id):
    """
    ...
    # create new tracks
    new_tracks = [{'bboxes': det['bbox'], 'original': det['bbox'], 'max_score': det['score'], 'life_time':1, 'loss_time': 0, 'id': 0, 'x_id': 0, 'x_flag': False} for det in dets]
    ...
    if len(x_tracks_index) == 2:
        ...
        index = max(distance(x_track1, x_track1), distance(x_track2, x_track2), distance(x_track1, x_track2), distance(x_track2, x_track1))
        if index < 2:
...
def distance(track1, track2):
    ...
    track2_start_x = (track2['original'][0] + track2['original'][1]) / 2
    track2_start_y = (track2['original'][2] + track2['original'][3]) / 2
...
def main():
    ...
    for folder in folders:
        ...
        for image in images:
            ...
            tracks_finished, tracks_active, id = track_iou(detections, tracks_active, 0.1, 0.2, 0.01, 5, id)
